...
def langevin(descriptor, x, num_steps, eps, sigma):
    for i in range(num_steps):
        ...
        x_feature.sum().backward()
        noise = torch.randn_like(x).to(device)
        x.data += eps * eps / 2 * (x.grad - x / sigma / sigma) + eps * noise
    return x


def run_expr(exp_dir: str, num_layers: int, img_path: str, logger: logging.Logger):
    if num_layers == 3:
        sigma = 1.0
    ...
    for sigma in [1.5,2.0, 1.0]:
        for langevin_step_size in [0.5, 1.0, 1.5]:
            print(sigma,langevin_step_size)
            ...
            for epoch in tqdm(range(num_epochs)):
                ...
                if (epoch + 1) % log_interval == 0:
                    logger.info(f"Epoch {epoch+1:<4d}: f_diff = {f_diff:<10.2f}, f_tgt = {f_tgt:<10.2f}, f_syn = {f_syn:<10.2f}")
                
            # visualize conv1 filters
            exp_prefix = exp_dir.split('_')[0]
            visualize_img_tensor(img_tensor=img_syn, img_path=os.path.join(exp_dir, f'{exp_prefix}_sigma{sigma}_LangevinStepsSize{langevin_step_size}.png'), mu=mu_tgt)
...
def run(exp_dir: str, num_layers: int, img_path: str, logger: logging.Logger):
    ...
    # visualize conv1 filters
    exp_prefix = exp_dir.split('_')[0]
    visualize_img_tensor(img_tensor=img_syn, img_path=os.path.join(exp_dir, f'{exp_prefix}_syn.png'), mu=mu_tgt)
    weights = list(model.parameters())[0].data
    grid = make_grid(weights, normalize=True)
    visualize_img_tensor(img_tensor=grid, img_path=os.path.join(exp_dir, 'conv1.png'))
...
def main():
    ...
    for img_file in img_files:
        if args.tag == os.path.splitext(img_file)[0]:
            ...
            #exp_dir = f'{args.tag}_{args.layer}layer_expr'
