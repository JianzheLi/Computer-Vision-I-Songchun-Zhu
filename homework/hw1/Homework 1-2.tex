...
\begin{center}
{\bf Computer Vision I: Low-Middle Level Vision  Homework Exercise \#1 }  \\ (total 7.5 points)   \\
...
\noindent{\bf Question 1}. (2 points) This exercise shows that the 1/f-power law
...
\noindent{\bf Question 2} (1.5  points). The goal of this exercise is to show the connection between the Gibbs/MRF model and partial differential equations (PDEs) for image processing.   Consider the continuous Gibbs/MRF model for a system in problem 1 again with potential function,
\[
    H(\I(x,y)) = \int\int (\nabla_x \I(x,y))^2 + (\nabla_y \I(x,y))^2 dxdy.
\]
...
\noindent{\bf Question 3}  (A scale invariant world, 1.5 points).
...
\noindent{\bf Question 4}  (Lagrange Mechanics, Least Action Principle, And Newton's Laws, 2.5 points).

First, we need to refresh our knowledge about Newton's laws which we learned in middle school. 
Let's recall what do the Newton's Laws tell us (from Wiki):
\begin{itemize}
    \item First law: Every object perseveres in its state of rest, or of uniform motion in a right line, except insofar as it is compelled to change that state by forces impressed thereon.
    \item Second Law: The change of motion of an object is proportional to the force impressed; and is made in the direction of the straight line in which the force is impressed.
    \item Third Law: To every action, there is always opposed an equal reaction; or, the mutual actions of two bodies upon each other are always equal, and directed to contrary parts.
\end{itemize}

Now, we will try to start with Lagrange Mechanics and the least action principle to rebuild Newton's Laws.
\begin{enumerate}
    \item  \emph{First Law:} The first law of motion is typically discussed within the context of the Inertial Frame of reference. Some researchers even regard this law as a defining criterion for identifying the Inertial Frame. In this context, we define the Inertial Frame as one in which both time and space are homogeneous. Given the homogeneity of space, the Lagrangian for a free particle is solely a function of the magnitude of its velocity, expressed as $\vec{v}^2$:
$$
L = L(\vec{v}^2) = \frac{1}{2} m \vec{v}^2
$$
\textbf{Based on this, please prove the first law of motion, which states that:}
$$
\vec{v} = const
$$


    \item \emph{Second Law:} In Lagrange Mechanics, we define \textbf{force} as:
    $$
    F := - \frac{\partial V}{\partial \vec{r}}
    $$
    where $V$ is the potential function, $\vec{r}$ is the coordinate of the particle.
    \textbf{Please prove the second law, which states that}:
    $$
    ...
    \item \emph{Third Law:} Now we are going from one particle to a set of particles, whose coordinates are depicted as $\vec{r}_1, \vec{r}_2, \ldots, \vec{r}_n$. We will first prove the conservation of momentum:
    \begin{itemize}
        \item Conservation of Momentum: By the homogeneity of space, we have under any small displacement, if the speed remains constant, the lagrangian remains unchanged. To be concrete:
        $$
        \delta L = \sum \limits_{i=1}^n \frac{\partial L}{\partial \vec{r}_i} \delta \vec{r}_i
        $$
        As the lagrangian remains constant for any small displacement, then we have:
        $$
        \sum \limits_{i=1}^n \frac{\partial L }{\partial \vec{r}_i} = 0
        $$
        If we define the momentum of a system as:
        $$
        \vec{P} := \sum \limits_{i=1}^n \frac{\partial L }{\partial \vec{v}_i}
        $$
        \textbf{Please prove that the momentum remains constant.}
        
        \item \textbf{Please prove the third law, which states mathematically:}
        $$
        \sum \limits_{i=1}^{n} m_i \frac{d\vec{v}_i}{dt} = 0
        $$
    ...
    By the homogeneity of time, the lagrangian doesn't depend on time, which states mathematically that:
    $$
    \frac{dL}{dt} = 0
    $$
    \textbf{Please prove the conservation of energy:}
    $$
