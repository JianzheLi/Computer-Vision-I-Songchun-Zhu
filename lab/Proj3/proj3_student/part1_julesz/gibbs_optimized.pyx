...
def gibbs_sample_cy_optimized(
    ...
    for s in range(sweep):
        ...
        error /= num_filters
        print(f'Gibbs Sweeps [{s+1}/{sweep}]: error = {error:.6f} max_error: {max_error:.6f}')
        
        # 退火
        current_T *= 0.96
        
        if max_error < 0.1:
            print(f"Gibbs Sweeps [{s+1}/{sweep}]: max_error: {max_error:.6f} < 0.1, stop!")
            break
...
@cython.boundscheck(False)
@cython.wraparound(False)
def pos_gibbs_sample_update_py(
    DTYPE_t[:, :] img_syn,
    DTYPE_t[:, :] hists_syn,
    DTYPE_t[:, :] hists_ori,
    list filter_list,
    ...
    DTYPE_t T):
    """
    Python-visible wrapper for pos_gibbs_sample_update_optimized
    """
    cdef int H = img_syn.shape[0]
    cdef int W = img_syn.shape[1]
    cdef int num_filters = hists_syn.shape[0]
    cdef int num_bins = hists_syn.shape[1]

    cdef DTYPE_t* img_ptr = &img_syn[0,0]
    cdef DTYPE_t* syn_ptr = &hists_syn[0,0]
    cdef DTYPE_t* ori_ptr = &hists_ori[0,0]
    cdef DTYPE_t* bounds_ptr = &bounds[0,0]
    cdef DTYPE_t* weight_ptr = &weight[0]
    cdef DTYPE_t* bin_ptr = &bin_widths[0]

    cdef int f_idx
    cdef DTYPE_t** filters = <DTYPE_t**>malloc(num_filters * sizeof(DTYPE_t*))
    cdef int* filter_sizes = <int*>malloc(num_filters * 2 * sizeof(int))

    cdef cnp.ndarray filter_np
    cdef DTYPE_t[:, :] filter_view
    ...
    cdef DTYPE_t norm_factor = 1.0 / (H * W)
    cdef unsigned int seed = (pos_h * 1315423911) ^ pos_w ^ 12345

    with nogil:
        pos_gibbs_sample_update_optimized(
            img_ptr, syn_ptr, ori_ptr,
            filters, filter_sizes,
            bounds_ptr, weight_ptr, bin_ptr,
            pos_h, pos_w,
            H, W, num_filters, num_bins,
            T, norm_factor, &seed)
