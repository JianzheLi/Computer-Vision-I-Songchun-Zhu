...
def parse_args():
    ...
    parser.add_argument('--start', default='cold', type=str, help='warm or cold')
    ...
    parser.add_argument('--n_stats', type=int, default=50, help='stats each n epochs')
...
def save_interp(model, args, z_std, device, path):
    ...
    #inter_number1 = torch.linspace(num_1_min, num_1_max, n_interp)
    #inter_number2 = torch.linspace(num_2_min, num_2_max, n_interp)
...
def run(args, exp_dir, logger):
    ...
    for epoch in tqdm(range(args.n_epochs)):
        ...
        if args.start == 'warm':
            ...
            z = sample_gaussian_prior(batch_size, args.nz, args.prior_sigma)
            z = model.sample_langevin(z, data)
        ...
        if (epoch + 1) % args.n_plot == 0:
            ...
            # random sample
            if args.start == 'warm':
                z_sampled = sample_gaussian_prior(batch_size, args.nz, args.prior_sigma)
            else:
                z_sampled = sample_gaussian_prior(batch_size, args.nz, args.prior_sigma)
